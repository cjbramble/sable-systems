...
def test_validation_disagreement_fails_without_changing_the_authored_labels(monkeypatch, tmp_path):
    monkeypatch.setattr(evaluation, "judge_answer", lambda *args: {"score": 1, "passed": True, "reason": "Always accepts"})
    code, report, _ = run_report(monkeypatch, tmp_path, {"mode": "validation"})
    assert code == 1
